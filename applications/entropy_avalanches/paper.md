...
We test that question. Our primary claim is a negative one: pretrained GPT-2-small activations on a real C4 corpus do not satisfy any of the four canonical mean-field DP signatures (α = 3/2, β = 2, γ ≈ 2, σ = 1). We corroborate this with a threshold plateau that does not plateau and a shuffle null that is not cleanly rejected.
...
No prior work we are aware of (i) runs the full CSN + Sethna + MR-σ + threshold-plateau + basis-invariance + shuffle-null battery (ii) on pretrained transformer LM activations on natural text (iii) against a matched-init random-weight control that preserves the architecture's own LayerNorm initialisation. Our contribution fills that specific niche.
...
1. **Power-law avalanche sizes** P(s) ∝ s^{−α} with α = 3/2 (mean-field prediction), fit via the Clauset-Shalizi-Newman protocol with goodness-of-fit p > 0.1 and alternative-distribution log-likelihood-ratio rejection of lognormal / exponential / truncated-power-law.
...
Following the Phase 0.25 publishability review for this project, we also require:
...
**Model.** GPT-2-small (124 M parameters, 12 transformer blocks, d_model = 768) from the HuggingFace hub, loaded in fp16 on a consumer RTX 3060 (12 GB).
...
**Power-law fit (α, β).** `powerlaw` v2.0 Python package with `discrete=True`, automatic `x_min` selection, and `distribution_compare` against lognormal, exponential, and truncated-power-law.
...
**MR-estimator σ.** `mrestimator` v0.2.0. Scalar activity signal a(t) = Σ_j |z_{t,j}|. Exponential autocorrelation fit with `kmax = 200`, `method = 'ts'`, `numboot = 100`. Branching ratio σ = `fit.mre`. Also applied on the reconstructed scalar of the top-K-PC subspace with K = round(participation ratio), for a Fontenele-style subspace signal.
...
Pipeline implemented in Python 3.12 with PyTorch 2.5.1+cu121, HuggingFace Transformers 5.5, `powerlaw` 2.0, `mrestimator` 0.2. SVD accelerated via `torch.linalg.svd` on CUDA (≈ 5–10× over NumPy at d = 768). All modules have pytest unit tests verified against synthetic branching-process ground truth before the real-data runs. A regression test suite accompanies the reference implementation.
...
**Power-law exponent α.** Across all 12 layers at θ = 2.5 ambient, α ∈ [2.63, 3.00] — well above the mean-field 3/2 prediction. At θ = 2.0 the range drops to [1.86, 2.98] but still never touches 3/2; at θ = 3.0 the range is [2.68, 3.00]. No θ, no layer, no basis reproduces α ≈ 3/2.
...
**Sethna γ.** γ_predicted = (β − 1)/(α − 1) ranges 0.36–1.14 across trained layers at θ = 2.5 ambient. The mean-field prediction γ = 2 is nowhere within reach; the measured γ (from ⟨s | T⟩ fits) similarly ranges 0.9–1.0. Sethna γ scaling relation does not close.
...
Every mean-field DP predicate fails. The picture is definitive: pretrained GPT-2-small activations on natural text are not in the mean-field DP criticality class.
...
Figure 3 (subspace-vs-ambient scatter) visualises the training-induced low-dimensionality.
...
At layer 6 trained θ = 2.5: ambient α = 2.84, random-rotation α = 3.00, PCA α = 2.53. The α statistic is robust under the three non-SAE bases we test, within the ±0.5 tolerance stated in our evidence bar. n_avalanches differs across bases (3 276 / 5 124 / 3 250) because thresholding at a fixed z-score captures different event sets in different bases; this is an expected, not pathological, basis effect.

### 4.5 Shuffle null does not cleanly reject

At θ = 2.5 ambient across trained layers: 2 of 8 fittable layers show `distinguishable = True` (L7 and L8, where the shuffled data failed to fit a power-law — so the distinction is trivially "real fits, shuffled doesn't"). The other 6 fittable layers show α_shuffled within ±0.3 of α_real. 4 layers produced too few avalanches for fitting on either real or shuffled.
...
Criticality claims for deep networks rest on a small number of studies [Morales et al. 2023, Wang et al. 2026] and a much larger body of adjacent work that uses "edge of chaos" as a loose characterisation rather than a quantitative prediction. A negative result on the quantitative mean-field DP predicates — across four independent signatures (α, β, γ, σ) with a threshold-plateau failure and a shuffle null that does not cleanly reject — is informative. It argues against casual adoption of cortical-avalanche exponents as hypotheses to confirm on trained LLMs, and it invites a different framework.
...
- **Single model scale.** GPT-2-small (124 M) only. Pythia-scale and training-trajectory sweeps are a companion project and not in scope here. Whether larger trained models reproduce the same depth-graded pattern is open.
...
- **Shape-collapse placeholder.** Full Papanikolaou 2011 per-avalanche profile collapse is not implemented; only the γ slope is reported. The paper's claim is not load-bearing on shape collapse, but the omission should be noted.
- **Corpus size.** 62 K valid tokens produces ≈ 10^3–10^4 avalanches per layer-threshold cell. Larger corpora would tighten CIs but are unlikely to change the qualitative conclusions given the size of the effects observed.

## 6 Conclusion

We applied a six-element statistical-physics battery to a pretrained transformer language model on natural text and found that pretrained GPT-2-small does not satisfy any of the mean-field directed-percolation criticality predicates (α ≈ 3/2, β ≈ 2, γ ≈ 2, σ ≈ 1). The threshold-plateau test fails by a large margin, and the temporal-shuffle null is not cleanly rejected on most layers. The picture is not that trained transformer activations are critical-with-caveats; it is that they are not critical.
...
Our recommendation for downstream work is twofold. First, criticality claims on trained-network activations should clear a threshold-plateau bar before they are taken seriously. Second, the depth-graded σ and PR signatures we report are candidate targets for scale-law studies (Pythia) and mechanistic-interpretability probes (path patching, circuit analysis); those would address whether the layer-specialisation pattern is universal or model-specific.

## 7 Data and code availability

A reference implementation of the pipeline, the 216-row results table, three figures, and a 14-test pytest regression suite will be archived at a Zenodo DOI prior to publication. An accompanying pre-registration document describes the Phase 0.25 scope-check reframes and the Phase 2.75 methodology-review revisions applied to the experiment design before the results reported here were generated.
...
(To be replaced by a numbered bibliography in `references.bib` at paper_submission stage. Inline citations in this draft resolve to entries in the parent ideation workspace `papers.csv` at `../entropy_ideation/papers.csv`.)
