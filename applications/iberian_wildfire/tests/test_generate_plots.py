...
def test_no_annotation_collisions(monkeypatch, tmp_path):
    ...
    This test captures all figures created during plot generation and
    checks for text bounding box overlaps.
    """
    captured_figs = []
    ...
    for fig in captured_figs:
        ...
        # Allow small overlaps from bar value labels
        significant = [(t1, t2) for t1, t2 in overlaps
                       if len(t1) > 3 and len(t2) > 3]
        assert len(significant) == 0, f"Text overlaps found: {significant[:5]}"
        plt.close(fig)
