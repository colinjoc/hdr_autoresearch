...
The parser is covered by a 15-test pytest suite (`tests/test_kat_parser.py`). The tests verify component counts against the canonical numbers in the Zoo solution README files, check that every parameter reference resolves to a defined parameter, and confirm that the parser does not crash on any of the 25 type8 solutions. All 15 tests pass.
