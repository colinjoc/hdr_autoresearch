...
Figure 1 (`plots/pred_vs_actual.png`) shows the predicted-vs-actual
scatter for the winning E08 model's 5-fold cross-validation
predictions. The overall trend follows the diagonal but with
substantial scatter at both extremes of the tensile-strength range,
consistent with the moderate R-squared of 0.625.
...
Figure 2 (`plots/feature_importance.png`) shows the XGBoost gain-
based feature importance for the winning E08 model trained on the
full dataset. Four of the five physics-informed features (infill
contact area, linear energy density, volumetric flow rate, and
thermal margin) rank among the top six most important features,
confirming that the derived features carry genuine predictive signal
that the raw features alone do not efficiently encode.
...
non-linear. Figure 4 (`plots/tree_to_linear.png`) shows the
tournament results as a bar chart with the 1.32x ratio annotated
alongside the 2x threshold reference line, confirming that the FDM
tensile-strength signal is mostly linear at N = 50 on this parameter
range.
...
Figure 3 (`plots/headline_finding.png`, left panel) visualises the
...
All figures are in the `plots/` directory, generated by
`generate_plots.py`. Style: seaborn-v0_8-whitegrid, colourblind-safe
(Wong 2011), 300 DPI.

- **Figure 1** (`plots/pred_vs_actual.png`): Predicted vs actual
  tensile strength scatter for the E08 winning model under 5-fold
  cross-validation (MAE = 4.29 MPa, R-squared = 0.625, N = 50).
- **Figure 2** (`plots/feature_importance.png`): XGBoost gain-based
  feature importance for the 14-feature E08 model, with the five
  physics-informed derived features highlighted in orange. Four of
  five physics features rank in the top six.
- **Figure 3** (`plots/headline_finding.png`): Two-panel headline
...
- **Figure 4** (`plots/tree_to_linear.png`): Phase 1 tournament bar
  ...
  the 2x threshold reference line, confirming the relationship is
  mostly linear at N = 50.
