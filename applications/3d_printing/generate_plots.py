...
def plot_pred_vs_actual(y_true: np.ndarray, y_pred: np.ndarray) -> None:
    ...
    ax.set_ylim(lo, hi)
    ax.set_aspect("equal")
    ax.set_xlabel("Actual Tensile Strength (MPa)", fontsize=14)
    ax.set_ylabel("Predicted Tensile Strength (MPa)", fontsize=14)
...
def plot_feature_importance(importance: pd.Series) -> None:
    ...
    ax.set_xlabel("Feature Importance (XGBoost gain)", fontsize=14)
    ...
    ax.legend(handles=[raw_patch, phys_patch], loc="lower right", fontsize=12)
...
def plot_headline_finding(results_df: pd.DataFrame) -> None:
    ...
    ax1.set_xlabel("5-fold CV MAE (MPa)", fontsize=14)
    ax1.set_ylabel("Experiment (sorted by MAE)", fontsize=14)
    ...
    fig.tight_layout(pad=2.0, w_pad=4)
...
def plot_tree_to_linear(results_df: pd.DataFrame) -> None:
    ...
    ax.set_ylabel("5-fold CV MAE (MPa)", fontsize=14)
    ...
    for bar, mae_val in zip(bars, maes):
        ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.05,
                f"{mae_val:.2f}", ha="center", va="bottom", fontsize=12,
                fontweight="bold")
    ...
    ax.text(2, mid_y + 0.15, f"Ratio = {ratio:.2f}x (threshold = 2.0x)",
            ha="center", va="bottom", fontsize=13, fontweight="bold", color=CB_RED)
