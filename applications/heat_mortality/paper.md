# Refuting the Night-Time Wet-Bulb Hypothesis: A Pre-Registered Hypothesis-Driven Test on 9,276 City-Weeks

## Abstract

Since Sherwood and Huber (2010) proposed a 35 degrees Celsius wet-bulb temperature as a theoretical upper limit for human thermoregulation, and particularly since Vecellio et al. (2022) measured an empirical critical wet-bulb temperature near 30.55 degrees Celsius in a controlled environmental chamber, a growing literature has argued that **night-time** wet-bulb temperature is the dominant predictor of heat-related mortality because overnight recovery is what determines whether a heat wave becomes lethal. The Lancet Countdown 2024-2025 and several recent papers (Achebak et al. 2022, Royé et al. 2021, He et al. 2022, Lee et al. 2025) have extended this to the population level, arguing that operational heat-health early warning systems should condition on the night-time wet-bulb temperature as a lead indicator. We pre-registered and tested this claim on 9,276 city-weeks across 30 United States and European Union cities, 2013-2025, using a Hypothesis-Driven Research (HDR) loop with 116 single-change experiments followed by 10 orthogonal robustness checks. The hypothesis is refuted. Of 22 flagship night-time wet-bulb hypotheses tested individually against a tight atmospheric baseline (ExtraTrees, Mean Absolute Error (MAE) 45.56 deaths per week, Coefficient of Determination (R-squared) 0.745, Area Under the Receiver Operating Characteristic curve (AUC-ROC) 0.822), exactly one kept: a 4-week rolling maximum of night-time wet-bulb temperature - a temporal memory signal, not a physiological threshold. Stacking all 22 flagship features hurts Mean Absolute Error. The AUC actually drifts slightly downward. None of ten robustness specifications (per-city ensembles, age-stratified proxy, Davies-Jones wet-bulb swap, alternative climatology, no fixed effects, no autocorrelation, hot cities only, heat-wave weeks only, a binary lethal-week classifier, and a Mediterranean-only subset) flip the negative finding. Phase B discovery confirms that the top feature by permutation importance is dry-bulb Tmax persistence, and a minimal 2-feature detector (dry-bulb Tmax exceedance streak + year index) reaches AUC 0.9696 for lethal-week classification - within the noise floor of the full model (AUC 0.9804). At a per-city top-5%-of-weeks alert rule, the literature's night-time wet-bulb threshold EWS configuration is **worse** (miss rate 62.5%) than the dry-bulb strawman (51.9%), while the HDR-recommended minimal detector achieves 38.0%. The policy implication is that heat-health early warning systems should focus development effort on dry-bulb extremes, temporal memory, and seasonality, not on the night-time wet-bulb threshold revision, at the population-week aggregation scale tested here.
...
This paper is a pre-registered empirical test of the population-scale claim, on a 9,276-city-week panel of 30 United States and European Union cities over 13 years. The test is pre-registered in the sense that the 116 hypotheses were specified in `research_queue.md` before the Phase 2 loop was run, and the 22 flagship night-time wet-bulb hypotheses were specified in order, with explicit keep-or-revert thresholds, before any of them were evaluated.

### 1.3 Pre-registered contributions

1. **A refutation of the night-time wet-bulb hypothesis at the population-week scale** on 30 cities x 13 years, robust to 10 orthogonal alternative specifications.
2. **A Phase 2 best-model configuration** that improves Mean Absolute Error from 47.49 deaths per week (baseline) to 40.33 deaths per week (a 15% relative improvement) through autocorrelation, distributed-lag dry-bulb Tmax, country fixed effects, and seasonality - **without any night-time wet-bulb feature in the winning set**.
3. **A Phase B discovery sweep** showing that a 2-feature minimal detector (dry-bulb Tmax streak + year index) achieves 98.9% of the full classifier's AUC, and that the literature's night-Tw threshold EWS rule is worse than a dry-bulb strawman at matched alert budget.
...
Weather data comes from the NOAA Integrated Surface Database (ISD) via `data_loaders.py`. Each city is assigned its primary airport Global Historical Climatology Network - Hourly (GHCN-H) station; hourly temperature, dewpoint, and pressure are aggregated to weekly via `aggregate_hourly_to_weekly`. Derived weekly columns include `tmax_c_mean`, `tmax_c_max`, `tmin_c_mean`, `tavg_c`, `tdew_c`, daytime `tw_c_mean` / `tw_c_max`, night-time `tw_night_c_mean` / `tw_night_c_max`, `rh_pct`, and a heat-wave indicator `consecutive_days_above_p95_tmax`. Wet-bulb is computed via Stull (2011) as the default; Davies-Jones (2008) is swapped in R03. Night is 22:00-06:00 local; alternative windows are tested as H011/H012.
...
All evaluation uses 5-fold `KFold(shuffle=True, random_state=42)` cross-validation. Metrics: Mean Absolute Error (MAE) and Root Mean Squared Error (RMSE) on `excess_deaths`, Coefficient of Determination (R-squared) on `excess_deaths`, AUC-ROC and Brier score on the lethal-heat-wave binary label, and MAE in p-score units (MAE_deaths / mean expected_baseline). The binary score is a logistic transform of the regression p-score proxy (`y_pred / expected_baseline`) centred at 0.10 with slope 10, giving a bounded pseudo-probability. The Phase 2.5 R09 variant replaces this with a genuine `ExtraTreesClassifier(class_weight='balanced')`.
...
| Metric | E00 baseline | Phase 2 best | Delta |
|---|---|---|---|
| MAE (deaths/week) | 47.488 | **40.334** | -15.1% |
| RMSE | 80.448 | 61.235 | -23.9% |
| R-squared | 0.6974 | 0.8247 | +12.7 percentage points |
| AUC-ROC (lethal) | 0.8277 | 0.8198 | **-0.79 percentage points** |
...
Phase 2 runs 116 pre-registered single-change hypotheses. The queue is assembled from `research_queue.md` and compiled in `phase2_runner.build_queue`. Each hypothesis specifies:
...
The headline Phase 2 question: of the 22 pre-registered night-time wet-bulb hypotheses (H001-H022), how many kept?
...
The literature's night-Tw threshold rule has both a **higher** false-alarm rate (3.93% vs 3.52%) and a **higher** miss rate (62.5% vs 51.9%) than a dry-bulb `tmax_c_max` rank-based strawman. The HDR minimal detector (which is `consecutive_days_above_p95_tmax` + `iso_year`, no night-Tw anywhere) dominates both. This is the operational claim of the paper: **swapping an operational EWS from dry-bulb Tmax to night-time Tw at the same alert budget increases the miss rate by ~10 percentage points and the false-alarm rate by ~0.4 percentage points**. At the 9,276-city-week panel scale, it is a strictly worse rule.

### 5.10 The headline

> **Night-time wet-bulb temperature does not explain population-week excess mortality variance that the dry-bulb baseline missed, on 9,276 city-weeks across 30 United States and European Union cities, 2013-2025. The negative finding holds across 10 orthogonal robustness specifications. The literature's proposed night-time wet-bulb threshold EWS rule is operationally dominated by a dry-bulb strawman. Heat-health early warning systems should focus development effort on dry-bulb extremes, temporal memory, and seasonality - not on the wet-bulb threshold revision, at this population-week aggregation scale.**
...
- **Label-feature coupling in R09** (Section 6.3): the binary saturation is partly a label-construction artefact. A stricter target (`p_score >= 0.10` without the meteorological gate) would lower the ceiling and give night-Tw features more room.
- **R02 70+ proxy is a linear rescaling**, not a true age split. Non-linear age x night-Tw interactions are invisible.
...
- **Achebak et al. (2022)** finds night-time heat carries mortality risk independent of daytime in Spanish cities. We do not reproduce this on the cross-country panel, even in the R10 Mediterranean-only subset. The mismatch is likely a difference in (a) spatial resolution, (b) statistical model (case time-series vs KFold regression), and (c) outcome definition.
...
- **Continue conditioning on daytime Tmax exceedance and its recent history**. The Phase 2 best features (Tmax lag 1-4, prior-week autocorrelation, country FEs, seasonality) are already in or adjacent to the current generation of EWS rules. The HDR minimal detector (`consecutive_days_above_p95_tmax` + year index) is a 2-scalar operational rule that dominates both the dry-bulb strawman and the literature night-Tw threshold rule at the same per-city 5%-of-weeks alert budget.
- **Do not swap to night-Tw as the primary indicator**. At matched alert budget, the literature night-Tw rule (`tw_night_c_max` rank) has a **higher** false-alarm rate and a **higher** miss rate than the dry-bulb strawman. Swapping an operational EWS from dry-bulb to night-Tw would hurt both public-health outcomes and the political capital of the EWS operator.
- **Investigate individual-level cohort studies with continuous exposure assessment** for a clean physiological threshold test. The population-week panel cannot resolve the hypothesis, but a minute-level cohort study could.
...
This project is a pre-registered test of the night-time wet-bulb hypothesis at the population-week scale on 9,276 city-weeks across 30 United States and European Union cities, 2013-2025. The hypothesis is refuted. One of 22 flagship night-time wet-bulb features kept in the Phase 2 single-change loop, and it is a 4-week rolling maximum - a memory signal, not a physiological threshold. Ten orthogonal robustness specifications do not flip the negative finding. The Phase 2 best model improves Mean Absolute Error from 47.49 to 40.33 deaths per week through autocorrelation, dry-bulb distributed lags, country fixed effects, and seasonality - features that have nothing to do with night-time wet-bulb temperature. Phase B discovery shows that a minimal 2-feature detector reaches AUC 0.9696 (98.9% of the full model) without any reference to night-Tw, and that the literature's night-Tw threshold EWS rule is operationally worse than a dry-bulb strawman at matched alert budget.

The recommendation to heat-health early warning system developers is to focus on dry-bulb extremes, temporal memory, and seasonality, and to treat the night-time wet-bulb threshold revision as an open hypothesis that requires individual-level cohort data to test properly, not population-week panel data.
...
*Full `results.tsv` (164 rows: E00, T01-T05, H001-H116, R01-R12 triplets, and B01-B05 Phase B rows), the Phase B discovery outputs, and the complete HDR methodology specification live in `applications/heat_mortality/` of the HDR autoresearch repository.*
